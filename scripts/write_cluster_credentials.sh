...
cat kubeconfig_$CLUSTER | opw write team-one-platform-${CLUSTER} kubeconfig -
...
# team-one-platform-nonprod-us-east-2
terraform output cluster_endpoint | tr -d \\n | sed 's/"//g' | opw write team-one-platform-${CLUSTER} cluster-endpoint -
terraform output cluster_certificate_authority_data | tr -d \\n | sed 's/"//g' | opw write team-one-platform-${CLUSTER} base64-certificate-authority-data -
